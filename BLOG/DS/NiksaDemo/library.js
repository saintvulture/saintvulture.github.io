...
function easeInOut(t, a, b) {
    // Ensure t is between 0 and 1
    t = Math.max(0, Math.min(1, t));

    // Apply ease-in-out function: smooth transition with acceleration and deceleration
    if (t < 0.5) {
        return a + (b - a) * (2 * t * t);  // Ease in (acceleration)
    } else {
        return a + (b - a) * (1 - Math.pow(-2 * t + 2, 2) / 2);  // Ease out (deceleration)
    }
}
