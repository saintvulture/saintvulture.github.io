...
function setup() {
  createCanvas(960, 640);
      ...
      skillNode[0] = new skillNode(perks.entry[0].ID, 50, 10+ 150*0);
  skillNode[1] = new skillNode(perks.entry[1].ID, 50, 10+ 150*1);
  skillNode[2] = new skillNode(perks.entry[2].ID, 50, 10+ 150*2);
  skillNode[3] = new skillNode(perks.entry[3].ID, 50, 10+ 150*3);
  
    
    
      skillNode[4] = new skillNode(perks.entry[4].ID, 250, 10+ 150*0);
      
      skillNode[5] = new skillNode(perks.entry[5].ID, 250, 10+ 150*1);
  
      skillNode[6] = new skillNode(perks.entry[6].ID, 250, 10+ 150*2);
  
      skillNode[7] = new skillNode(perks.entry[7].ID, 250, 10+ 150*3);
...
function mousePressed() {
  ...
  mousePosY = mouseY
    ...
    let hover =
            mouseX >= 0 && mouseX <= 0 + img.width &&
            mouseY >= 400 && mouseY <= 400 + img.height;
  
  if (hover && mouseButton === RIGHT) {
    console.log('attack Melee')
    }
    if (hover && mouseButton === LEFT) {
    console.log('attack Magic')
    }
...
//PERK FUNCTIONS
function funcPerkMaenad(){
    //console.log("Maenad activated")
}

function funcPerkTheTower(){
    //console.log("Maenad activated")
  armorRating = armorRating * (skillNodeTest3.num)
  ...
  skillNodeDraw() {
    ...
    for (let i = 0; i < perks.entry.length; i++) {
      if (perks.entry[i].ID === this.ID) {
        ...
        ellipse(inputCircleX, inputCircleY, 40, 2+40); //inbound circle large
          
        }
         else {
            ellipse(inputCircleX, inputCircleY, 20, 20); //inbound circle small
        }
        ...
        rect(this.posX, this.posY, this.boxScaleX, this.boxScaleY); // overall Box
        rect(this.posX, this.posY, this.boxScaleX, 20); // name part box
        ...
        for (let j = 0; j < nodeInstances.length; j++) {   //loop through nodeInstances to find input connector
          ...
          if (dist(circleX, circleY, mouseX, mouseY) <= 20 && isMousePressed) {
            ellipse(circleX, circleY, 40, 40); //"highlights" the circle the mouse is near
          ...
          rect(this.posX + i * (this.boxScaleX / this.outNodes), this.boxScaleY + this.posY, this.boxScaleX / this.outNodes, 40);
        }
        ...
        // Skill name
        text(perks.entry[i].skill + ": " + perks.entry[i].perkName, this.posX + 5, this.posY + 5, this.boxScaleX - 5, this.boxScaleY - 5);

        // Skill Text, we use a replacer to indicate the variable num
        
        
        
        text(perks.entry[i].text.replace("{{num}}", this.num), this.posX + 5, this.posY + 25, this.boxScaleX - 5, this.boxScaleY - 5);
        ...
        for (let j = 0; j < this.outNodes; j++) {
          text(perks.entry[i].outBoundNode[j].outEffect, this.posX + 5 + j * (this.boxScaleX / this.outNodes), this.posY + this.boxScaleY + 5, this.boxScaleX - 5, this.boxScaleY - 5);

          text(perks.entry[i].outBoundNode[j].outClass, this.posX + 5 + j * (this.boxScaleX / this.outNodes), this.posY + 15 + this.boxScaleY + 5, this.boxScaleX - 5, this.boxScaleY - 5);
        }
      }
      // BOX DESIGN
    }
...
//SKILLS
// WARRIOR: Armor, Melee, Ranged
// MAGE: Destruction, Restoration, Thaumaturgy
// ROGUE: Speechcraft, Mobility, Survival